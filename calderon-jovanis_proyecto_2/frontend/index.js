((Utils) => {
    const App = {
        htmlElements: {
            ...
            pokemonButtons: document.querySelector("#buttonDiv"),
        ...
        handlers: {
            pokemonFinderFormOnSubmit: async (e) => {
                ...
                if(e.target.id === "btnClean"){
                    const buttonTemplate = App.templates.buttonTemplate2();
                    App.htmlElements.pokemonFinderOutput.innerHTML = "";
                    App.htmlElements.pokemonFinderOutput.removeAttribute("class");
                    App.htmlElements.pokemonButtons.className = "bottonContainer";
                    App.htmlElements.pokemonButtons.innerHTML = buttonTemplate;
                    App.htmlElements.pokemonFinderInput.value = "";
                    App.htmlElements.pokemonFinderSearchType.value = "pokemon";
                }
                ...
                if(e.target.id === "btnSearch"){
                    ...
                    try {
                        const responseAux = await Utils.getPokemon({query, searchType});
                        ...
                        if(searchType === "pokemon"){
                            ...
                            evolution = App.templates.evolutionChainList(evolutionChain.chain);
                        }
                        const renderedTemplate = App.templates.render({searchType, response}, evolution);
                        const buttonTemplate = App.templates.buttonTemplate();
                        App.htmlElements.pokemonFinderOutput.className = "outputBox";
                        App.htmlElements.pokemonFinderOutput.innerHTML = renderedTemplate;
                        App.htmlElements.pokemonButtons.className = "bottonContainer2";
                        App.htmlElements.pokemonButtons.innerHTML = buttonTemplate;
                    } catch(error) {
                        App.htmlElements.pokemonFinderOutput.className = "outputBox";
        ...
        templates: {
            render: ({searchType, response}, evolution) => {
                const renderMap = {
                    ability: App.templates.abilityCard,
                    pokemon: App.templates.pokemonCard
                };
                return renderMap[searchType] ? renderMap[searchType](response, evolution): App.templates.errorCard();
            },
            errorCard: () => `<h1>There was an error</h1>`,
            pokemonCard: ({id, name, weight, height, sprites, abilities}, evolution) => {
                let nameUpper = Utils.toUpperCaseFirst(name);
            ...
            abilityCard: ({name, pokemon}) => {
                const pokemonList = pokemon.map(
                    ({ pokemon, is_hidden }) =>
                    `<li>${pokemon.name} ${is_hidden ? App.htmlElements.svgEye : ""}</li>`
                );
                return `<h1>${name}</h1>
                        <div class="row">
                            <div class="column">
                                <div>
                                    <h2>Who can learn it?</h2>
            ...
            buttonTemplate: () => {
                return `<button type="button" class="button" id="btnClean">Clean</button>
                        <button type="button" class="button">Search</button>`;
            },
            buttonTemplate2: () => {
                return `<button type="submit" class="button" id="btnSearch">Search</button>`;
            },
            evolutionChainList: ({species, is_baby, evolves_to}, item = "") => {
                let itemAux = item + `<li>${species.name} ${is_baby ? App.htmlElements.svgBaby : ""}</li>`;
                return ((brk, arry, chain) => {
                    return brk ? itemAux: App.templates.evolutionChainList(arry, chain);
                })(evolves_to[0] === undefined, evolves_to[0], itemAux);
            }
        }
    }
    App.init();
